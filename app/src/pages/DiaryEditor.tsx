...
const buildFormData = useCallback(async () => {
  ...
  return form;
    ...
    useEffect(() => {
        const id = setInterval(async () => {
            if (!dirtyRef.current || !title.trim()) return;
            const form = await buildFormData();
            if (!form) return;
            try {
                const response = await saveDiary(form);
    ...
    return (
        <div className="page-sheikah font-orbitron relative min-h-screen flex flex-col">
            <div className="absolute top-5 left-5 z-20">
                <button onClick={handleBack} className="btn-zelda-apple">返回</button>
            </div>
            <div className="absolute top-5 right-5 z-20 flex gap-2">
                <button onClick={generatePreview} className="btn-zelda-apple">预览</button>
                <button onClick={() => setSidebarOpen(!isSidebarOpen)} className="btn-zelda-apple">属性</button>
                {/* 新增的临时保存按钮 */}
                <button onClick={() => handleSave()} className="btn-zelda-apple">
                    测试保存
                </button>
            ...
            <main className="flex-grow w-full h-full relative">
                <ErrorBoundary>
                    <div className="absolute inset-0 z-5">
                        <Excalidraw
                            // --- 关键修复 4: 使用 excalidrawAPI prop 代替 ref ---
            ...
            <aside className={`absolute top-0 right-0 h-full bg-[#f8f1d5]/80 backdrop-blur-sm shadow-lg p-4 z-30 transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full'}`} style={{width: '300px'}}>
                <h2 className="text-xl font-bold mb-4 text-center text-[#3e6c4e]">日记属性</h2>
                <div className="space-y-6">
                    <div>
                        <label className="block text-[#3e6c4e] mb-2">心情</label>
                        <MoodSelector mood={mood} onChange={onMoodChange} />
                    </div>
                    <div>
                        <label className="block text-[#3e6c4e] mb-2">标签</label>
                        <div className="taginput-zelda">
                            {tags.map(tag => (
                                <span key={tag} className="taginput-tag flex items-center gap-1">
                                    {tag}
                                    <button onClick={() => removeTag(tag)} className="text-red-300 hover:text-red-500 font-bold bg-transparent">×</button>
                                </span>
                            ))}
                            <input
                                type="text"
            ...
            {previewImage && (
                <div className="absolute top-10 left-10 w-80% max-w-800px bg-white border-2 border-blue-500 rounded-xl overflow-hidden shadow-lg z-100 p-4">
                    <img src={previewImage} alt="预览图" className="w-full h-auto object-contain" />
                    <button onClick={() => setPreviewImage(null)} className="mt-2 p-2 bg-red-500 text-white rounded">关闭预览</button>
                </div>
            )}
        </div>
    );
